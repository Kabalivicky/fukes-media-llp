...
const LoadingIntro = () => {
  ...
  useEffect(() => {
    ...
    if (prefersReducedMotion) {
      // Skip flicker entirely for reduced motion
      setPhase('reveal');
      return;
    }
    ...
    // Reduce flicker interval from 80ms to 120ms for smoother performance
    ...
    <AnimatePresence>
      {shouldShow && (
        <motion.div
          ...
          style={{ zIndex: 10000, willChange: 'opacity' }}
          initial={{ opacity: 1 }}
        ...
        >
          ...
          {/* Film grain — use CSS opacity transition instead of re-renders */}
          {!prefersReducedMotion && (
            <div
              ...
              style={{
                ...
                opacity: phase === 'flicker' ? 0.06 : 0.03,
              }}
            />
          )}

          {/* Dust particles — reduced count */}
          {!prefersReducedMotion && <DustParticles isPreWipe={isPreWipe} />}

          {/* Lens flare */}
          {!prefersReducedMotion && <LensFlare phase={phase} />}

          {/* Logo section — click to enter */}
          <LogoReveal phase={phase} onEnter={handleEnter} reducedMotion={prefersReducedMotion} />

          {/* Wipe transition */}
          {phase === 'wipe' && !prefersReducedMotion && <WipeTransition />}
          ...
          >
            <div className="h-[2px] bg-white/10 rounded-full overflow-hidden">
              <motion.div
                ...
                style={{
                  background: `linear-gradient(90deg, ${BRAND.red}, ${BRAND.blue}, ${BRAND.green})`,
...
const FlickerOverlay = ({ phase, flickerOpacity, countdownNumber }: { phase: string; flickerOpacity: number; countdownNumber: number }) => {
  ...
  return (
    <>
      ...
      {/* Countdown */}
      <motion.div className="absolute flex items-center justify-center" style={{ width: 180, height: 180, zIndex: 50, willChange: 'transform, opacity' }}
      ...
      >
        ...
        <motion.div className="absolute inset-0 rounded-full"
          style={{ border: '3px solid transparent', borderTopColor: `rgba(255,255,255,${flickerOpacity * 0.8})`, willChange: 'transform' }}
          animate={{ rotate: 360 }}
...
// Reduced particle count from 20 → 10 for mobile perf
const DustParticles = ({ isPreWipe }: { isPreWipe: boolean }) => {
  const particles = useMemo(() => [...Array(10)].map((_, i) => ({
    id: i,
    x: Math.random() * 100, y: Math.random() * 100,
    size: Math.random() * 3 + 1, opacity: Math.random() * 0.3 + 0.1,
...
const LogoReveal = ({ phase, onEnter, reducedMotion }: { phase: string; onEnter: () => void; reducedMotion: boolean }) => (
  <motion.div
    ...
    transition={{ duration: reducedMotion ? 0.3 : 1, delay: phase === 'reveal' ? 0.2 : 0, ease: [0.16, 1, 0.3, 1] }}
  >
    {/* RGB glow behind logo — use CSS animations for GPU acceleration */}
    {!reducedMotion && (
      <div className="absolute" style={{ width: 420, height: 420 }}>
        <div className="absolute inset-0 rounded-full" style={{
          ...
          filter: 'blur(60px)',
          animation: 'rgb-orb-drift-1 6s ease-in-out infinite',
          willChange: 'transform, opacity',
        }} />
        <div className="absolute inset-0 rounded-full" style={{
          background: `radial-gradient(circle at 70% 75%, ${BRAND.blue}, ${BRAND.blue}80 30%, transparent 70%)`,
          filter: 'blur(60px)',
          animation: 'rgb-orb-drift-2 8s ease-in-out infinite',
          willChange: 'transform, opacity',
        }} />
        <div className="absolute inset-0 rounded-full" style={{
          background: `radial-gradient(circle at 50% 85%, ${BRAND.green}, ${BRAND.green}80 30%, transparent 70%)`,
          filter: 'blur(60px)',
          animation: 'rgb-orb-drift-3 10s ease-in-out infinite',
          willChange: 'transform, opacity',
        }} />
      </div>
    )}

    {/* Spinning RGB border ring — use CSS animation instead of framer-motion for GPU */}
    <div
      className="absolute rounded-full"
      style={{
        width: 'calc(clamp(14rem, 22vw, 18rem) + 16px)',
        height: 'calc(clamp(14rem, 22vw, 18rem) + 16px)',
        zIndex: 9,
    ...
    {/* Tagline */}
    <motion.p className="mt-6 text-xs md:text-sm text-white/40 tracking-[0.3em] md:tracking-[0.5em] uppercase"
      initial={{ opacity: 0 }}
...
const WipeTransition = () => (
  <>
    ...
    />
    <motion.div className="absolute inset-0 bg-white" style={{ zIndex: 70, willChange: 'opacity' }}
      initial={{ opacity: 0 }}
