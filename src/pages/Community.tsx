...
import { 
  ...
  MessageCircle, Play, Heart, Eye, ExternalLink, ArrowRight,
...
// Sample community members for demonstration
const sampleMembers = [
  {
    id: '1',
...
const Community = () => {
  ...
  // Query for real profiles from database
  ...
  const { data: dbPortfolio, isLoading: loadingPortfolio } = useQuery({
    queryKey: ['community-portfolio'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('portfolio_items')
        .select(`
          *,
          user:profiles!portfolio_items_user_id_fkey(display_name, avatar_url)
        `)
